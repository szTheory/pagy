...
class Pagy
  ...
  module Frontend
    ...
    def pagy_nav(pagy)
      ...
      p_prev = pagy.prev
      p_next = pagy.next
      html   = if p_prev then %(<span class="page prev">#{link.call p_prev, pagy_t('pagy.nav.prev'), 'aria-label="previous"'}</span> )
      ...
      html << if p_next then %(<span class="page next">#{link.call p_next, pagy_t('pagy.nav.next'), 'aria-label="next"'}</span>)
    ...
    def pagy_info(pagy, item_name=nil)
      count = pagy.count
      key   = if    count == 0      then 'pagy.info.no_items'
