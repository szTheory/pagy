...
class Pagy
  ...
  def initialize(vars)
    ...
    { count:0, items:1, outset:0, page:1 }.each do |name,min|
  ...
  def series(size=@vars[:size])
    ...
    series = []
    [*0..size[0], *@page-size[1]..@page+size[2], *@last-size[3]+1..@last+1].sort!.each_cons(2) do |left, right|
      next if left.negative? || left == right || left > @last         # skip out of range and duplicates
      if    left+1 == right  then series.push(left)                   # no gap     -> no additions
      elsif left+2 == right  then series.push(left, left+1)           # 1 page gap -> fill with missing page
